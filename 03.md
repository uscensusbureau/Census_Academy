...
Let's go through how we would find the dataset (i.e., API endpoint) and variable ID to make an API call in a step by step fashion:
...
2. Dig through the available 2016 endpoints until we find the one we're looking for (`acs/acs1`) and click the `examples` link to see how calls are made at all the geographic levels available for that endpoint
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwMjkyMzI2NjYsLTE1ODE4NTI3OTFdfQ
==
-->