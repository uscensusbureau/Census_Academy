...
2. Dig through the available 2016 endpoints until we find the one we're looking for (`acs/acs1` - note that Detailed Tables are where the "estimates" reside, whereas the other `acs1...` endpoints are specifically noted as: `cprofile`) and click the [`examples`](https://api.census.gov/data/2016/acs/acs1/examples.html) link to see how calls are made at all the geographic levels available for that endpoint.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTQxODQ0MTM3NSwtMTU4MTg1Mjc5MV19
-->